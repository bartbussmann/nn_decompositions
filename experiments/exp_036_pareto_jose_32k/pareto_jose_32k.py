"""Pareto plot for jose's target model (t-9d2b8f02) using naturally-trained checkpoints.

Same as exp_035 but with 32k dict_size models on jose's target model with transcoders/CLTs from
mats-sprint/pile_local_sweep_jose_32k. The artifact structure differs: each TC run
trains all 4 layers (artifacts named layer{i}_final), whereas exp_011 had
one layer per run.

Usage:
    python experiments/exp_036_pareto_jose_32k/pareto_jose_32k.py
"""
...
# =============================================================================
# Artifact downloading (new structure: all layers in one run)
# =============================================================================
...
def download_transcoders(project: str) -> dict[int, dict[int, Path]]:
    """Download transcoder artifacts from pile_local_sweep_jose_32k.

    Each TC run (e.g. tc_k8) trains all 4 layers and logs 4 artifacts:
      tc_k8_checkpoint_layer0_final, tc_k8_checkpoint_layer1_final, ...
    ...
    for run in runs:
        ...
        assert top_k is not None, f"No top_k in config for run: {name}"

        arts = [a for a in run.logged_artifacts() if a.type == "model"]
        layer_arts = {}
        for a in arts:
            # Artifact names like "tc_k8_checkpoint_layer0_final:v0"
        ...
        layer_paths = {}
        for layer_idx in LAYERS:
            dest = Path(f"checkpoints/jose32k_tc_{name}_layer{layer_idx}")
...
def download_clts(project: str) -> list[tuple[int, Path]]:
    """Download CLT artifacts from pile_local_sweep_jose_32k.

    Each CLT run (e.g. clt_k8) logs one artifact: clt_k8_checkpoint_final.
    ...
    for run in runs:
        ...
        arts = [a for a in run.logged_artifacts() if a.type == "model"]
        # CLT has a single "final" artifact
        ...
        assert len(final_arts) == 1, (
            ...
            f"{[a.name for a in final_arts]}"
        )

        dest = Path(f"checkpoints/jose32k_clt_{name}")
...
def main():
    parser = argparse.ArgumentParser(description="Pareto plot for jose's target model")
    parser.add_argument("--project", type=str, default="mats-sprint/pile_local_sweep_jose_32k")
    ...
    parser.add_argument("--save_path", type=str,
                        default="experiments/exp_036_pareto_jose_32k/output/pareto_jose_32k.png")
    args = parser.parse_args()

    # Load SPD model (includes jose's base LlamaSimpleMLP)
    ...
    # Download transcoders and CLTs from same project
