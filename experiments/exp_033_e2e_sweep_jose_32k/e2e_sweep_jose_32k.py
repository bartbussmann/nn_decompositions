...
WANDB_PROJECT = "pile_e2e_sweep_jose_32k"
LAYERS = [0, 1, 2, 3]
DICT_SIZE = 32768
NUM_TOKENS = int(5e8)
LR = 3e-4
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="E2E training sweep (jose target model, dict_size=32768) with GPU queue")
    ...
    print(f"\n=== E2E Sweep (jose, dict_size=32768): {len(jobs)} jobs ===")
    for j in jobs:
        print(f"  {j.name}")
