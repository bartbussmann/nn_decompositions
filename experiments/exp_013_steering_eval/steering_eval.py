...
def judge_concept_score(
    client: OpenAI, model: str, concept_description: str, text: str,
) -> float:
    """Score how strongly the text reflects the target concept (0-2 scale)."""
    prompt = (
        f"Given the concept description: '{concept_description}'\n\n"
        f"Rate how strongly this text reflects that concept on a 0-2 scale:\n"
        f"0 = not at all related to the concept\n"
        f"1 = somewhat related or tangentially mentions the concept\n"
        f"2 = strongly reflects or is clearly about the concept\n\n"
        f"Text: \"{text}\"\n\n"
...
def main():
    ...
    total_evals = sum(
        len(gens)
        for concept_gens in all_generations.values()
        for gens in concept_gens.values()
    )
    pbar = tqdm(total=total_evals, desc="LLM judging")
