...
Same analysis as exp_011 but on jose's target model with transcoders/CLTs from
mats-sprint/pile_local_sweep_jose. The artifact structure differs: each TC run
trains all 4 layers (artifacts named layer{i}_final), whereas exp_011 had
one layer per run.

Usage:
    python experiments/exp_035_pareto_jose/pareto_jose.py
"""
...
def download_transcoders(project: str) -> dict[int, dict[int, Path]]:
    """Download transcoder artifacts from pile_local_sweep_jose.
    ...
    for run in runs:
        ...
        assert set(layer_arts.keys()) == set(LAYERS), (
            f"Expected artifacts for all layers in run {name}, got layers {set(layer_arts.keys())}"
        )
...
def download_clts(project: str) -> list[tuple[int, Path]]:
    """Download CLT artifacts from pile_local_sweep_jose.
...
def main():
    ...
    parser.add_argument("--save_path", type=str,
                        default="experiments/exp_035_pareto_jose/output/pareto_jose.png")
