"""Local-MSE Pareto plot: 4k + 32k dict sizes on the base LLM.

Single figure showing CE / MSE vs three capacity definitions for the
locally-trained PLT (BatchTopK Transcoders) and CLTs at both 4k and
32k dict sizes, overlaid with VPD baselines (three CI thresholds) and
the neuron baseline. Reads from the `pile_local_sweep_jose*` wandb
projects by default.

Usage:
    python experiments/pareto_plot_local/pareto_plot_local.py
    python experiments/pareto_plot_local/pareto_plot_local.py --plot-only
"""
